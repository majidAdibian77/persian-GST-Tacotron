...
from datasets import blizzard, ljspeech, blizzard2013, persian_data
...
def preprocess_blizzard2013(args):
  in_dir = os.path.join(args.base_dir, 'data')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = blizzard2013.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)

def preprocess_persian(args):
  in_dir = os.path.join(args.base_dir, 'data')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = persian_data.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def main():
  ...
  parser.add_argument('--dataset', required=True, choices=['persian', 'blizzard', 'ljspeech', 'blizzard2013'])
  ...
  elif args.dataset == 'persian':
    preprocess_persian(args)
